package com.github.gustavovitor.erriaga.config.security.filters;
...
public class RefreshTokenPostProcessor implements ResponseBodyAdvice<OAuth2AccessToken> {
    ...
    * Este cookie expira em 30 dias.*/
    private void addRefreshTokenIntoCookie(String refreshToken, HttpServletRequest request, HttpServletResponse response) {
        ...
        refreshTokenCookie.setSecure(apiConfig.getSecurity().isEnableHttps());
        refreshTokenCookie.setPath(request.getContextPath() + "/oauth/token");
        refreshTokenCookie.setMaxAge(apiConfig.getSecurity().getRefreshTokenValiditySeconds());
